...
function buildDedupeKey(error: Error, extra?: Record<string, unknown>): string {
  const source = typeof extra?.['source'] === 'string' ? (extra!['source'] as string) : 'unknown'
  const message = error.message || 'unknown'
  const topStackLine = (error.stack || '').split('\n')[1] || ''
  const componentStack = typeof extra?.['componentStack'] === 'string' ? (extra!['componentStack'] as string) : ''
