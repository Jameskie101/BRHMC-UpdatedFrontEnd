...
const DoctorSidebar = () => {
  ...
  const onZoomChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(v)) setZoom(Math.min(Math.max(v, 30), 200));
  };
      ...
      {isModalOpen && (
        <>
          <style>{`
            ...
            @keyframes pfwFade { from{opacity:0} to{opacity:1} }
            @keyframes pfwSlide {
            ...
            .pfw-modal-title i { color: #0d6efd; font-size: 15px; }
            ...
            .pfw-nav-list {
              flex: 1; overflow-y: auto; padding: 8px 0; margin: 0; list-style: none;
            }
            .pfw-nav-list::-webkit-scrollbar { width: 4px; }
            .pfw-nav-list::-webkit-scrollbar-thumb { background: #2a2e33; border-radius: 2px; }
            ...
            .pfw-print-btn {
              ...
              font-size: 11px; width: 100%; padding: 7px 0; text-align: center;
              transition: color .12s, background .12s;
            }
            .pfw-print-btn:hover { color: #e2e8f0; background: rgba(255,255,255,.05); }
            .pfw-print-btn i { font-size: 18px; }
            ...
            .pfw-zoom-input {
              width: 44px; border: none; outline: none;
            ...
            .pfw-zoom-input::-webkit-inner-spin-button { -webkit-appearance: none; }
            .pfw-zoom-spins {
              display: flex; flex-direction: column;
              border-left: 1px solid #ced4da;
            }
            .pfw-zoom-spin {
              flex: 1; background: #f8f9fa; border: none; cursor: pointer;
              font-size: 7px; color: #495057;
              padding: 0 5px; line-height: 1;
              display: flex; align-items: center; justify-content: center;
              transition: background .1s;
            }
            .pfw-zoom-spin:hover { background: #e2e6ea; }
            .pfw-zoom-spin + .pfw-zoom-spin { border-top: 1px solid #ced4da; }
            .pfw-toolbar-label { font-size: 12.5px; color: #6c757d; }
            ...
            .pfw-frame-area::-webkit-scrollbar { width: 8px; height: 8px; }
            .pfw-frame-area::-webkit-scrollbar-track { background: #e9ecef; }
            ...
            .pfw-loading-area {
              ...
              gap: 12px; color: #6c757d; font-size: 13px;
              background: #f0f2f5;
            }
            .pfw-spinner {
          ...
          {/* Backdrop */}
          <div className="pfw-modal-backdrop" onClick={() => setIsModalOpen(false)} />
            ...
            >
              ...
              <div className="pfw-modal-body">
                ...
                <div className="pfw-nav-col">
                  <ul className="pfw-nav-list" role="tablist" aria-label="Printable forms">
                    {FORM_TABS.map((tab) => (
                      <li key={tab.id} role="presentation">
                        <button
                          type="button"
                          role="tab"
                          className={`pfw-nav-btn${activeTab === tab.id ? " active" : ""}`}
                ...
                <div className="pfw-content-col">
                  ...
                  <div className="pfw-toolbar">
                    <div className="pfw-zoom-wrap">
                      ...
                      <div className="pfw-zoom-spins">
                        <button type="button" className="pfw-zoom-spin" onClick={zoomIn}  aria-label="Zoom in">▲</button>
                        <button type="button" className="pfw-zoom-spin" onClick={zoomOut} aria-label="Zoom out">▼</button>
                      </div>
                    </div>
                  ...
                  {/* Form preview */}
                  {isLoading ? (
                    <div className="pfw-loading-area" role="status" aria-live="polite">
                    ...
                    <div className="pfw-frame-area">
                      ...
                      >
                        <iframe
                          ...
                          title={FORM_TABS.find(t => t.id === activeTab)?.label ?? "Form"}
